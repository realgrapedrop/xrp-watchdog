...
Based on the successful XRPL Validator Monitor deployment at https://xrpl-monitor.grapedrop.xyz
...
┌─────────────────────────────────────────────────────────┐
│                  Public Internet                        │
│              https://xrpl-monitor.grapedrop.xyz         │
└────────────────────────┬────────────────────────────────┘
                         │
                         │ HTTPS
                         ↓
...
- ✅ Clean URL stays in address bar (`https://xrpl-monitor.grapedrop.xyz`)
...
2. **Domain DNS** managed by Cloudflare (grapedrop.xyz)
...
services:
  xrpl-monitor-grafana-prod:
    ...
    environment:
      ...
      GF_SERVER_ROOT_URL: "https://xrpl-monitor.grapedrop.xyz"
...
```bash
cloudflared tunnel route dns xrpl-monitor xrpl-monitor.grapedrop.xyz
```
...
ingress:
  - hostname: xrpl-monitor.grapedrop.xyz
...
routes = [
  { pattern = "xrpl-monitor.grapedrop.xyz/*", zone_name = "grapedrop.xyz" }
]
```
...
- `zone_name`: Root domain (grapedrop.xyz)
...
#   xrpl-monitor.grapedrop.xyz/* (zone name: grapedrop.xyz)
```
...
curl https://xrpl-monitor.grapedrop.xyz/health
# Expected: {"status":"ok"}
```
...
# HTML response
curl https://xrpl-monitor.grapedrop.xyz/ | grep iframe
# Should show iframe with dashboard path
```

### Step 4: Browser Test

1. **Open:** `https://xrpl-monitor.grapedrop.xyz`
...
# Should show:
# Route: xrpl-monitor.grapedrop.xyz/*
# Zone: grapedrop.xyz
```
...
```bash
curl https://xrpl-monitor.grapedrop.xyz/ | grep -o 'src="[^"]*"'
...
dig xrpl-monitor.grapedrop.xyz

# Should show CNAME pointing to tunnel
nslookup xrpl-monitor.grapedrop.xyz
```
...
# Configuration
DOMAIN="xrpl-monitor.grapedrop.xyz"
GRAFANA_PORT="3004"
...
- **XRPL Validator Monitor:** https://xrpl-monitor.grapedrop.xyz
