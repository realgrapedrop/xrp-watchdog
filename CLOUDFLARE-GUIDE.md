# Cloudflare Public Dashboard Deployment Guide

**Comprehensive guide for deploying a Grafana dashboard publicly using Cloudflare Tunnel and Workers**

Based on the successful XRP Watchdog deployment at https://xrp-watchdog.grapedrop.xyz
...
│  • Routes traffic to localhost:PORT                     │
...
│  • Login form disabled                                  │
│  • Embedding allowed                                    │
│  • Connects to data sources (ClickHouse, etc.)          │
└─────────────────────────────────────────────────────────┘
```
...
- ✅ Clean URL stays in address bar (`https://dashboard.domain.com`)
...
- Data source configured (ClickHouse, Prometheus, etc.)
...
**File:** `compose/prod-grafana-dashboard/docker-compose.yaml`
...
services:
  grafana-prod:
    ...
    container_name: grafana-prod-dashboard
    network_mode: host
    ...
    environment:
      ...
      GF_SERVER_HTTP_PORT: "3002"  # Change this to your port
      GF_SERVER_ROOT_URL: "https://your-dashboard.domain.com"
      ...
      # Default Dashboard
      GF_DASHBOARDS_DEFAULT_HOME_DASHBOARD_PATH: "/etc/grafana/provisioning/dashboards/your-dashboard.json"
    ...
    volumes:
      - /home/user/monitoring/data/grafana-prod-dashboard:/var/lib/grafana
      - /home/user/monitoring/provisioning/prod-dashboard:/etc/grafana/provisioning:ro
      - /home/user/monitoring/compose/prod-grafana-dashboard/custom.css:/usr/share/grafana/public/css/custom.css:ro
      - /home/user/monitoring/compose/prod-grafana-dashboard/index.html:/usr/share/grafana/public/views/index.html:ro
...
| Variable | Purpose |
|----------|---------|
| `GF_SERVER_HTTP_PORT` | Port Grafana listens on (change as needed) |
| `GF_SERVER_ROOT_URL` | Public URL for your dashboard |
...
**File:** `compose/prod-grafana-dashboard/custom.css`
...
```bash
# Extract the default index.html from Grafana 11.2.0
docker run --rm grafana/grafana:11.2.0 cat /usr/share/grafana/public/views/index.html > index.html
```
...
**File:** `provisioning/prod-dashboard/dashboards/dashboards.yaml`
...
`provisioning/prod-dashboard/dashboards/your-dashboard.json`

#### 1.6 Datasource Provisioning Configuration

Configure your data source (example uses ClickHouse - adjust for your needs):

**File:** `provisioning/prod-dashboard/datasources/clickhouse.yaml`
...
**File:** `provisioning/prod-dashboard/datasources/prometheus.yaml`
...
```bash
cd /home/user/monitoring/compose/prod-grafana-dashboard
docker compose up -d
...
cloudflared tunnel create your-dashboard

# Output will show tunnel ID:
# Created tunnel your-dashboard with id a1aec802-96a1-4b36-b87f-0bfcf169c213
...
```bash
cloudflared tunnel route dns your-dashboard your-dashboard.domain.com
```
...
**File:** `~/.cloudflared/config-your-dashboard.yml`
...
ingress:
  - hostname: your-dashboard.domain.com
    service: http://127.0.0.1:3002  # Your Grafana port
...
- `service`: Must match your Grafana port
...
```bash
sudo tee /etc/systemd/system/cloudflared-your-dashboard.service > /dev/null <<'EOF'
[Unit]
Description=Cloudflare Tunnel (your-dashboard)
...
Group=user
ExecStart=/usr/local/bin/cloudflared tunnel --config /home/user/.cloudflared/config-your-dashboard.yml run
Restart=always
RestartSec=3
...
sudo systemctl enable cloudflared-your-dashboard
sudo systemctl start cloudflared-your-dashboard

# Check status
systemctl status cloudflared-your-dashboard
...
```bash
mkdir -p /home/user/monitoring/workers/your-dashboard/src
cd /home/user/monitoring/workers/your-dashboard
```
...
export default {
  async fetch(req, env, ctx) {
    ...
    if (url.pathname === "/" || url.pathname === "/index.html") {
      // IMPORTANT: Update these to match your dashboard
      const dashboardPath = "/d/your-dashboard-uid/your-dashboard-slug";
...
1. **Dashboard Path:** Update `dashboardPath` to match your dashboard:
   - Format: `/d/<dashboard-uid>/<dashboard-slug>`
   - Find in Grafana: Dashboard → Settings → General → UID
   - Example: `/d/xrp-watchdog/xrp-watchdog`
...
3. **Title:** Change `<title>Your Dashboard</title>`
...
```toml
name = "your-dashboard-kiosk"
main = "src/index.js"
compatibility_date = "2025-11-07"

routes = [
  { pattern = "your-dashboard.domain.com/*", zone_name = "domain.com" }
]
```

**Important:**
- `name`: Worker name (shows in Cloudflare dashboard)
- `pattern`: Must match your full subdomain
- `zone_name`: Your root domain
...
# Uploaded your-dashboard-kiosk (3.15 sec)
# Deployed your-dashboard-kiosk triggers (1.68 sec)
#   your-dashboard.domain.com/* (zone name: domain.com)
```
...
curl https://your-dashboard.domain.com/health
# Expected: {"status":"ok"}
```
...
# Dashboard exists
curl http://localhost:3002/api/dashboards/uid/your-dashboard-uid | jq .

# Anonymous auth enabled
curl http://localhost:3002/api/user | jq .
...
# Check logs
sudo journalctl -u cloudflared-your-dashboard -f
```
...
# HTML response
curl https://your-dashboard.domain.com/ | grep iframe
# Should show iframe with dashboard path
```

### Step 4: Browser Test

1. **Open:** `https://your-dashboard.domain.com`
...
curl -s http://localhost:3002/api/datasources | jq '.[] | {name, uid, type}'

# Check dashboard datasource references
curl -s http://localhost:3002/api/dashboards/uid/your-dashboard-uid | \
...
```bash
sudo journalctl -u cloudflared-your-dashboard -n 50
```
...
| "Connection refused" | Grafana not running on specified port |
| "Failed to register" | DNS not configured correctly |

**Restart tunnel:**

```bash
sudo systemctl restart cloudflared-your-dashboard
```
...
# Workers & Pages → your-dashboard-kiosk → Settings → Triggers

# Should show:
# Route: your-dashboard.domain.com/*
# Zone: domain.com
```
...
```bash
curl https://your-dashboard.domain.com/ | grep -o 'src="[^"]*"'
# Should show: src="/d/your-dashboard/slug?kiosk&refresh=10s"
```
...
```bash
docker exec grafana-prod-dashboard env | grep AUTH
# Should show:
# GF_AUTH_ANONYMOUS_ENABLED=true
# GF_AUTH_DISABLE_LOGIN_FORM=true
```
...
```bash
systemctl status cloudflared-your-dashboard | grep "Registered tunnel connection"
```
...
1. Check config file syntax:
   ```bash
   cat ~/.cloudflared/config-your-dashboard.yml
   ```
...
3. Check cloudflared logs:
   ```bash
   sudo journalctl -u cloudflared-your-dashboard -n 100
   ```
...
cp /path/to/dashboard.json /home/user/monitoring/provisioning/prod-dashboard/dashboards/
...
```bash
# 1. Edit src/index.js
vi /home/user/monitoring/workers/your-dashboard/src/index.js
...
# Change: image: grafana/grafana:11.2.0
#     To: image: grafana/grafana:11.3.0
...
```bash
# Grafana logs
docker logs grafana-prod-dashboard -f
...
# Workers & Pages → your-dashboard-kiosk → Logs → Real-time logs
```
...
# Workers & Pages → your-dashboard-kiosk → Logs
...
docker exec grafana-prod-dashboard env | grep EMBEDDING
...
<div style="display:flex;align-items:center;margin-top:10px;font-size:1.3rem;line-height:1.8;">
  ...
  <div style="display:flex;align-items:center;flex-wrap:wrap;gap:8px;">
    <a href="/d/your-dashboard-uid/your-dashboard-slug?var-window=5m&from=now-5m&to=now"
       style="color:#4da3ff;text-decoration:none;margin:0 6px;font-size:1.3rem;font-weight:500;">5m</a> ▪
    <a href="/d/your-dashboard-uid/your-dashboard-slug?var-window=1h&from=now-1h&to=now"
       style="color:#4da3ff;text-decoration:none;margin:0 6px;font-size:1.3rem;font-weight:500;">1h</a> ▪
    <a href="/d/your-dashboard-uid/your-dashboard-slug?var-window=6h&from=now-6h&to=now"
       style="color:#4da3ff;text-decoration:none;margin:0 6px;font-size:1.3rem;font-weight:500;">6h</a> ▪
    <a href="/d/your-dashboard-uid/your-dashboard-slug?var-window=24h&from=now-24h&to=now"
       style="color:#4da3ff;text-decoration:none;margin:0 6px;font-size:1.3rem;font-weight:500;">24h</a> ▪
    <a href="/d/your-dashboard-uid/your-dashboard-slug?var-window=2d&from=now-2d&to=now"
       style="color:#4da3ff;text-decoration:none;margin:0 6px;font-size:1.3rem;font-weight:500;">2d</a> ▪
    <a href="/d/your-dashboard-uid/your-dashboard-slug?var-window=7d&from=now-7d&to=now"
       style="color:#4da3ff;text-decoration:none;margin:0 6px;font-size:1.3rem;font-weight:500;">7d</a> ▪
    <a href="/d/your-dashboard-uid/your-dashboard-slug?var-window=14d&from=now-14d&to=now"
       style="color:#4da3ff;text-decoration:none;margin:0 6px;font-size:1.3rem;font-weight:500;">14d</a> ▪
    <a href="/d/your-dashboard-uid/your-dashboard-slug?var-window=30d&from=now-30d&to=now"
...
- **Dashboard UID:** Find in Dashboard Settings → General → UID
- **Dashboard Slug:** The URL-friendly version of your dashboard title
...
```
/d/your-dashboard-uid/your-dashboard-slug?var-window=24h&from=now-24h&to=now
│   │                 │                    │          │              │
...
**Finding Your Dashboard UID and Slug:**

1. Open your dashboard
2. Click the gear icon (⚙️) → Settings
3. Look at the URL or the General tab:
   - **UID:** Shown in "UID" field (e.g., `xrp-watchdog`)
   - **Slug:** Shown in URL (e.g., `/d/xrp-watchdog/xrp-watchdog`)
...
- **Color:** Change `color:#4da3ff` to match your theme (try `#33cc33` for green, `#ff6b6b` for red)
- **Spacing:** Adjust `margin` and `gap` values for tighter/looser layout
- **Bullet separators:** The `▪` character can be changed to `|`, `•`, or removed entirely
...
<div>
  ...
  <a href="/d/uid/slug?from=now-1h&to=now">Last Hour</a><br>
  <a href="/d/uid/slug?from=now-24h&to=now">Last Day</a><br>
  <a href="/d/uid/slug?from=now-7d&to=now">Last Week</a>
</div>
```
...
<div style="display:flex;gap:10px;">
  <a href="/d/uid/slug?from=now-1h&to=now"
     style="padding:8px 16px;background:#0066cc;color:white;border-radius:4px;text-decoration:none;">1 Hour</a>
  <a href="/d/uid/slug?from=now-24h&to=now"
     style="padding:8px 16px;background:#0066cc;color:white;border-radius:4px;text-decoration:none;">24 Hours</a>
</div>
```
...
export default {
  async fetch(req, env, ctx) {
    ...
    // Route based on path
    const dashboards = {
      "/": "/d/main-dashboard/main?kiosk",
      "/analytics": "/d/analytics-dashboard/analytics?kiosk",
      "/status": "/d/status-dashboard/status?kiosk"
    };
...
```bash
#!/bin/bash
# deploy-dashboard.sh - Automated dashboard deployment
...
journalctl -u cloudflared-<name> -f
```
...
- **XRP Watchdog Dashboard:** https://xrp-watchdog.grapedrop.xyz
